...
emit('log', '1. [setup] 컴포넌트가 메모리에 생성되었습니다. (DOM 접근 불가능)')

// 부착 (Mounting) 단계
onMounted(() => {
  emit('log', '2. [onMounted] 화면에 완벽히 부착되었습니다! (API 호출/DOM 조작 적기)')
...
onUnmounted(() => {
  ...
  emit('log', '4. [onUnmounted] 컴포넌트가 소멸했습니다. 타이머 청소 완료!')
})
</script>
...
<template>
  <div class="lifecycle-child">
    <p>
      ⏱️ 3초마다 자동으로 올라가는 count: <strong>{{ count }}</strong>
    </p>
  </div>
</template>
